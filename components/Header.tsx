...
export const Header: React.FC<HeaderProps> = ({ theme, toggleTheme }) => {
  ...
  const isAmbientDark = theme === 'dark' || (location.pathname === '/' && !isScrolled);

  const textColorClass = isAmbientDark ? 'text-white' : 'text-slate-900 dark:text-white';
  ...
  const navLinkClass = (href: string) => {
    ...
    if (isAmbientDark) {
      return isActive 
        ? 'text-blue-400 border-blue-500/20 bg-blue-500/10 shadow-[0_0_15px_rgba(59,130,246,0.15)]' 
        : 'text-white/70 border-white/[0.06] hover:text-white hover:border-white/20 hover:bg-white/5';
    } else {
      return isActive 
        ? 'text-blue-600 border-blue-600/20 bg-blue-50 dark:bg-blue-950/20 shadow-[0_2px_15px_rgba(37,99,235,0.05)]' 
        : 'text-slate-500 border-slate-200 dark:border-slate-800 hover:text-slate-950 dark:hover:text-white hover:border-slate-300 dark:hover:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-900/40';
    }
  };

  const logoBgClass = isAmbientDark ? 'bg-blue-600 text-white' : 'bg-slate-900 dark:bg-blue-600 text-white';

  const themeButtonClass = isAmbientDark
    ? 'text-white/70 hover:text-white hover:bg-white/10'
    : 'text-slate-500 dark:text-slate-400 hover:text-slate-950 dark:hover:text-white hover:bg-slate-100 dark:hover:bg-slate-800';

  return (
    <header 
      className={`fixed top-0 left-0 right-0 z-[100] transition-all duration-500 ease-out border-b font-normal ${
        isScrolled || location.pathname !== '/'
          ? 'bg-white/90 dark:bg-slate-950/95 backdrop-blur-3xl border-slate-200/50 dark:border-white/[0.08] py-3 shadow-[0_4px_30px_rgba(0,0,0,0.03)] dark:shadow-[0_4px_40px_rgba(0,0,0,0.4)]' 
    ...
    >
      <div className="container mx-auto px-6 flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          >
            <div className={`rounded-xl flex items-center justify-center shadow-lg transition-all duration-500 group-hover:scale-110 group-hover:rotate-3 ${isScrolled ? 'w-9 h-9' : 'w-10 h-10'} ${logoBgClass}`}>
              {logoUrl ? (
            ...
            <span className={`font-medium tracking-tight transition-all duration-300 ${textColorClass} ${isScrolled ? 'text-lg' : 'text-xl'}`}>
              {COMPANY_NAME}
            </span>
          </Link>
        ...
        <nav className="hidden lg:flex items-center gap-6" aria-label="Main Navigation">
          ...
          <div className={`ml-4 pl-4 border-l flex items-center gap-2 ${isAmbientDark ? 'border-white/10' : 'border-slate-200 dark:border-slate-800/60'}`}>
             <button
        ...
        <div className="flex items-center gap-2 md:hidden">
          ...
          <button 
            className={`w-10 h-10 flex items-center justify-center rounded-xl transition-all ${isAmbientDark ? 'text-white' : 'text-slate-900 dark:text-white'}`}
